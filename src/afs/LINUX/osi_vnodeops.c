                           ...
                           struct afs_pagecopy_task *task)
{
    ...
    if (!code) {
        if (folio_test_uptodate(cachefolio)) {
            ...
            /* Queue for background copy - convert to pages for existing API */
...
static int
afs_begin_writeback(struct vcache *vcp, cred_t **acredp)
{
    int code;
...
static int
afs_end_writeback(struct vcache *vcp, cred_t **acredp, int written_size, unsigned int requested_size)
{
    int code;
...
afs_linux_writefolio_cb(struct folio *folio, struct writeback_control *wbc, void *priv)
{
    ...
    code = afs_begin_writeback(vcp, &credp);
    if (code == AOP_WRITEPAGE_ACTIVATE) {
	return code;
    }
    ...
    code1 = afs_end_writeback(vcp, &credp, code, to);
